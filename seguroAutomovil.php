            ...
            <div class="row">
                <table class="table table-bordered wow zoomIn" data-wow-duration=".6s" data-wow-delay=".3s">
                  ...
                  <?php
                        ...
                        $query = "SELECT pol.id, emp.firstName, pago.paymentType, seguro.secureType, banco.bankName, pol.cost, pol.paymentDate FROM poliza pol INNER JOIN empleado emp on pol.employeesId = emp.id INNER JOIN pago on pol.paymentId = pago.id INNER JOIN seguro on pol.insuranceId = seguro.id INNER JOIN banco on pol.bankId = banco.id WHERE seguro.secureType='Auto'";
